...
public sealed class TrafficViewModel : ViewModelBase
{
    ...
    private async Task PollAsync()
    {
        while (true)
        {
            try
            {
                var s = await _metrics.GetSnapshotAsync(CancellationToken.None);
                if (s != null)
                {
                    Dispatcher.UIThread.Post(() =>
                    {
                        Snapshot = s;
                        Codes.Clear();
                        foreach (var kv in s.Codes.OrderByDescending(kv => kv.Value).Take(6)) Codes.Add(new CodeCount{ Code = kv.Key.ToString(), Count = kv.Value});
                        TopHosts.Clear();
                        foreach (var kv in s.Hosts.OrderByDescending(kv => kv.Value.Req).Take(10)) TopHosts.Add(new HostReq{ Host = kv.Key, Req = kv.Value.Req});
                    });
                }
            }
            catch { }
