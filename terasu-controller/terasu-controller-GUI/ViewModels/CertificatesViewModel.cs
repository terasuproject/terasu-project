...
namespace terasu_controller_GUI.ViewModels
{
    public sealed class CertificatesViewModel : ViewModelBase
    {
        ...
        {
            ProxyConfig cfg = _cfg.Load();
            CertPath = cfg.Ca.CertFile;
            Generate = ReactiveCommand.CreateFromTask<System.Reactive.Unit, System.Reactive.Unit>(async _ =>
            {
                await _ca.EnsureFilesAsync(cfg.Ca.CertFile, cfg.Ca.KeyFile);
                Thumbprint = await _ca.GetThumbprintAsync(cfg.Ca.CertFile) ?? string.Empty;
                return System.Reactive.Unit.Default;
            });
            Install = ReactiveCommand.CreateFromTask<System.Reactive.Unit, System.Reactive.Unit>(async _ =>
            {
                await _ca.InstallAsync(cfg.Ca.CertFile);
                return System.Reactive.Unit.Default;
            });
            _ = Generate.Execute();
        }
