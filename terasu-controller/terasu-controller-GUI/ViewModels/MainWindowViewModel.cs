...
namespace terasu_controller_GUI.ViewModels
{
    public class MainWindowViewModel : ViewModelBase
    {
        ...
        {
            Tabs.Add(new Models.TabPage
            {
                Title = "Dashboard",
                Content = new DashboardViewModel(BaseAddress)
            });
            Tabs.Add(new Models.TabPage
            {
                Title = "Logs",
                Content = new LogsViewModel(BaseAddress)
            });
            Tabs.Add(new Models.TabPage
            {
                Title = "Traffic",
                Content = new TrafficViewModel(BaseAddress)
            });
            Tabs.Add(new Models.TabPage
            {
                Title = "Rules",
                Content = new RulesViewModel()
            });
            Tabs.Add(new Models.TabPage
            {
                Title = "Settings",
            ...
            {
                Title = "Certificates",
                Content = new CertificatesViewModel()
            });
            Tabs.Add(new Models.TabPage
            {
                Title = "Diagnostics",
                Content = new DiagnosticsViewModel()
            });
            Selected = Tabs[0];
        }
    }
