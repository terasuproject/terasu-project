...
ProxyConfig cfg = cfgMgr.LoadOrDefault();

string binPath = Environment.GetEnvironmentVariable("TERASU_PROXY_BIN")
                 ?? Path.Combine(AppContext.BaseDirectory, "terasu-proxy");

ProxyManager proxy = new(binPath, cfgMgr.ConfigPath);
CaManager ca = new();
HealthClient health = new(cfg.Metrics.Addr.StartsWith("http") ? cfg.Metrics.Addr : $"http://{cfg.Metrics.Addr}");
...
switch (args[0])
{
    case "start":
    {
        ...
        await proxy.StartAsync(cfg.Dns.Mode, true);
    ...
    case "install-ca":
    {
        ...
        bool ok = await ca.InstallAsync(cfg.Ca.CertFile);
    ...
    case "uninstall-ca":
    {
        string? thumb = await ca.GetThumbprintAsync(cfg.Ca.CertFile);
        bool ok = await ca.UninstallAsync(thumbprint: thumb);
    ...
    case "set-mode":
    {
        if (args.Length < 2)
        {
            Console.WriteLine("need value: all|list");
            return;
        }
        cfg.Mode = args[1];
        cfgMgr.Save(cfg);
    ...
    case "list-domains":
    {
        foreach (string d in cfg.InterceptList) Console.WriteLine(d);
        break;
    }
    ...
    case "set-dns-mode":
    {
        if (args.Length < 2)
        {
            Console.WriteLine("need value: auto|terasu|system");
            return;
        }
        cfg.Dns.Mode = args[1];
        cfgMgr.Save(cfg);
    ...
    {
        await ca.EnsureFilesAsync(cfg.Ca.CertFile, cfg.Ca.KeyFile);
        string? tp = await ca.GetThumbprintAsync(cfg.Ca.CertFile);
    ...
    case "enable-basic-auth":
    {
        if (args.Length < 3)
        {
            Console.WriteLine("need: <user> <pass>");
            return;
        }
    ...
    case "set-listen":
    {
        if (args.Length < 2)
        {
            Console.WriteLine("need: <host:port>");
            return;
        }
        cfg.Listen = args[1];
        cfgMgr.Save(cfg);
